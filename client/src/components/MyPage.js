...
export default function MyPage(){
    ...
    const handleSignOut = async () => {
        ...
        if(window.confirm("PenPick 서비스를 탈퇴 하시겠습니까? (가지마...)")){
            try {
                const response = await axios.delete('http://localhost:8282/api/user/signOut', {
                    withCredentials: true,
                });
            ...
            <main id="myPage-layout">
                ...
                <section id="myPage-content">
                    <div>
                        ...
                        <span style={{color:"orangered", fontWeight:"bold"}}>{userInfo.nickname}</span><span>님의 회원 정보</span>
                    </div>
                    <hr />
                    {isEditing ? (
                        ...
                        <DatePicker
                            id="user-birth-edit"
                            selected={editedUserInfo.birthday ? new Date(editedUserInfo.birthday) : null}
                            onChange={(date) => setEditedUserInfo((prevUser) => ({ ...prevUser, birthday: date }))}
                            dateFormat="yyyy-MM-dd"
                            showYearDropdown
                            placeholderText="  생일을 선택해주세요"
                        /><br />
                        <button id="saveEditInfo" onClick={handleSave}>변경 정보 저장</button>
                    </div>
                    ) : (
                        // 수정 완료 상태 (DB에 저장된 값)
                        <div>
